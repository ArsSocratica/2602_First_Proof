...
outcomes: six problems received correct answers with complete
(though often methodologically distinct) proofs; one received a
valid approach to an open-ended question; two received correct
answers with incomplete proofs; and one addressed a different case
than the one asked.  We analyze the failure modes, compare our
outputs with single-prompt LLM baselines from the organizers'
internal testing, and identify cross-field tool discovery as the
principal bottleneck for AI-assisted mathematical research.
...
\paragraph{Complete proof with correct answer (6/10).}
Problems P01, P02, P03, P05, P08, and P09.  In each case, our
...
In six of ten problems, the iterative multi-model workflow
...
For P01, our proof includes an analysis of why the HKN
set~\cite{HKN24} fails to support the $\Phi^4_3$ measure---a
contribution absent from the official solution.

\subsection{Failure mode analysis}

The three problems with incomplete or divergent results
(P04, P06, P07) share a common structural feature: the
official proof requires importing a specific tool from an
\emph{adjacent} mathematical field that is not suggested by
...
This pattern suggests that \textbf{cross-field tool discovery}
is the principal bottleneck for current AI systems in
research-level mathematics.  The models excelled at deep
exploration within a single mathematical field but could not
make the non-obvious connections between fields that
characterize expert mathematical insight.
...
\begin{enumerate}[nosep]
\item Six of ten problems received correct answers with
  complete proofs, often via methodologically distinct
  approaches.
\item The three incomplete or divergent results share a common
  failure mode: the official proof required importing tools
  from an adjacent mathematical field not suggested by the
  problem statement.
\item Iterative multi-model collaboration avoided the failure
  modes observed in single-prompt LLM baselines, but did not
  overcome the cross-field discovery bottleneck.
\item A non-expert human operator can orchestrate AI systems
  to produce research-level mathematical proofs, but the
  quality ceiling is determined by the AI systems' ability
  to discover non-obvious cross-field connections.
\end{enumerate}
...
official solutions, this assessment is inflated: the honest
count is 6/10 complete, 1/10 valid approach, 2/10 incomplete,
and 1/10 divergent.  We report this discrepancy as a cautionary
note on the reliability of AI self-assessment in mathematical
contexts.
