...
Our workflow achieves 7/10 correct answers (6~complete proofs plus
1~valid approach), with 2~incomplete proofs and 1~divergent result.
...
\paragraph{Ours: 6 complete + 1 valid approach + 2 incomplete + 1 wrong case.}
We produce correct, complete proofs for P01, P02, P03, P05, P08,
and P09.  For P10, we provide a valid PCG method.  We produce
incomplete proofs for P04 ($n \leq 3$ only) and P06 (partial
cases, no universal constant).  For P07, we prove NO for the
wrong case ($\delta(G)=0$ instead of 2-torsion).
...
\subsection{P01: $\Phi^4_3$ measure shift}
...
All three proofs are complete and use different separating
functionals.
...
\item Both AI submissions achieve 7--8/10 correct answers, but
...
\item Problem~7 is a shared failure and the most important
  cautionary finding: current AI systems can produce elaborate,
  internally coherent, but ultimately incorrect proofs at
  research level---a failure mode more dangerous than obviously
  broken arguments.
\item Cross-field tool discovery remains the principal bottleneck
  for AI-assisted research mathematics.
