...
\title{\textbf{Problem 9 --- Quadrilinear Determinantal Tensors}}
\author{}
\date{}
...
Let $n \geq 5$. Let $A^{(1)}, \ldots, A^{(n)} \in \R^{3 \times 4}$ be Zariski-generic. For $\alpha, \beta, \gamma, \delta \in [n]$, let $Q^{(\alpha\beta\gamma\delta)} \in \R^{3 \times 3 \times 3 \times 3}$ with entries
\[
Q^{(\alpha\beta\gamma\delta)}_{ijkl} = \det\bigl[A^{(\alpha)}(i,:);\; A^{(\beta)}(j,:);\; A^{(\gamma)}(k,:);\; A^{(\delta)}(l,:)\bigr].
\]
Then there exists a polynomial map $\F\colon \R^{81n^4} \to \R^N$ such that:
\begin{enumerate}
\item $\F$ does not depend on $A^{(1)}, \ldots, A^{(n)}$.
\item The degrees of the coordinate functions of $\F$ are at most $4$, independent of $n$.
...
\begin{itemize}[nosep]
\item $a^{(\alpha)}_i \in \R^4$: $i$-th row of camera $A^{(\alpha)}$, for $i \in [3] := \{1,2,3\}$.
...
\item Non-identical: not all four camera indices equal.
\item For $\{p,q\} \subset \{1,2,3,4\}$, write $\{r,s\} = \{1,2,3,4\} \setminus \{p,q\}$ for the complementary positions.
\end{itemize}

%----------------------------------------------------------------------
\section[Construction of F]{Construction of $\F$}\label{sec:construction}
%----------------------------------------------------------------------
...
\end{equation}
The map $\F$ is the collection of all $F^{\{p,q\}}$ equations over all six pairs $\{p,q\}$, all camera choices, and all row-index pairs.

\medskip
\noindent\textbf{Explicit instantiation for $\{p,q\} = \{1,2\}$.}\quad Complement $\{r,s\} = \{3,4\}$. Shared cameras $(c_1,c_2) = (\alpha,\beta)$. Varying cameras $(v_1,v_2,v_3,v_4) = (\gamma,\delta,\gamma',\delta')$. Row indices $(i,j,k,l,k',l') \in [3]^6$. Then:
\begin{alignat*}{2}
...
%----------------------------------------------------------------------
\section[Necessity]{Necessity ($\lambda$ rank-1 $\Rightarrow$ $\F = 0$)}
\label{sec:necessity}
%----------------------------------------------------------------------
...
\begin{proof}
This is the Pl\"ucker relation for $\Gr(2,4)$; see \cite{GH78}, Ch.~1, \S5, eq.~(5.1), p.~211, or \cite{HP94}, Vol.~I, Ch.~VII, \S4, Theorem~I.
\end{proof}
...
With shared cameras $(\alpha,\beta)$ in positions $(1,2)$ and varying cameras $(\gamma,\delta,\gamma',\delta')$ in positions $(3,4)$, Lemma~\ref{lem:plucker} with $r_1 = a^{(\alpha)}_i$, $r_2 = a^{(\beta)}_j$ gives at the $Q$-level:
\[
...
%----------------------------------------------------------------------
\section[Sufficiency: Pairwise Rank-1]{Sufficiency --- Pl\"ucker Equations Imply Pairwise Rank-1}
%----------------------------------------------------------------------

\begin{proposition}\label{prop:pairwise}
For Zariski-generic cameras, if $F^{\{p,q\}} = 0$ for all camera and row-index choices, then for every pair $\{p,q\}$ and every choice of cameras $c_p, c_q$, the matrix $M^{(c_p,c_q)}_{v_r, v_s} := \lambda_{(c_p, c_q;\, v_r, v_s)}$ has rank $1$ in $(v_r, v_s)$.
\end{proposition}
...
For any $n \geq 5$ and any choice of cameras $\gamma, \delta, \gamma', \delta'$ (with possible repetitions among $\alpha, \beta$), the bracket in $(\dagger)$ is a polynomial in the camera entries that is not identically zero.
\end{lemma}

\begin{proof}
It suffices to exhibit one camera configuration where the bracket is nonzero (since a polynomial that is nonzero at a point is nonzero on a Zariski-open dense set). Consider cameras $A^{(m)}$ with rows
\[
...
\medskip
\noindent\emph{Proof of Proposition~\ref{prop:pairwise} (continued).}\quad Since the bracket is a nonzero polynomial, it is nonzero on a Zariski-open dense subset of the camera parameter space. Combined with $\Lambda_P \neq 0$ (all $\lambda$-values on non-identical tuples are nonzero), we conclude from $(\dagger)$:
\begin{equation}\tag{$\star_{12}$}
...
\begin{itemize}[nosep]
\item $(\star_{12})$: For fixed $(\alpha,\beta)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\gamma,\delta)$.
\item $(\star_{13})$: For fixed $(\alpha,\gamma)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\beta,\delta)$.
\item $(\star_{14})$: For fixed $(\alpha,\delta)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\beta,\gamma)$.
\item $(\star_{23})$: For fixed $(\beta,\gamma)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\alpha,\delta)$.
\item $(\star_{24})$: For fixed $(\beta,\delta)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\alpha,\gamma)$.
\item $(\star_{34})$: For fixed $(\gamma,\delta)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\alpha,\beta)$.
\end{itemize}

%----------------------------------------------------------------------
\section[Sufficiency: Rank-1]{Sufficiency --- Pairwise Rank-1 Implies Rank-1}
%----------------------------------------------------------------------

\begin{proposition}\label{prop:rank1}
If $\lambda_{\alpha\beta\gamma\delta} \neq 0$ for all non-identical $(\alpha,\beta,\gamma,\delta)$ and the six conditions $(\star_{12})$--$(\star_{34})$ hold, then there exist $u, v, w, x \in (\R^*)^n$ such that $\lambda_{\alpha\beta\gamma\delta} = u_\alpha v_\beta w_\gamma x_\delta$ for all non-identical $(\alpha,\beta,\gamma,\delta)$.
\end{proposition}
...
\medskip\noindent\textbf{Step 1.}\quad By $(\star_{12})$, for each fixed $(\alpha,\beta)$, the matrix $\lambda_{\alpha\beta\gamma\delta}$ has rank~1 in $(\gamma,\delta)$ on non-identical tuples. So:
\[
...
\medskip\noindent\textbf{Step 2.}\quad By $(\star_{23})$, for each fixed $(\beta,\gamma)$, $\lambda_{\alpha\beta\gamma\delta}$ is rank-1 in $(\alpha,\delta)$. Substituting Step~1: $\varphi(\alpha,\beta,\gamma) \cdot \psi(\alpha,\beta,\delta)$ must be rank-1 in $(\alpha,\delta)$ for fixed $(\beta,\gamma)$. The $\varphi$-factors cancel (nonzero), leaving:
\[
...
\medskip\noindent\textbf{Step 3.}\quad By $(\star_{14})$, for fixed $(\alpha,\delta)$, $\lambda$ is rank-1 in $(\beta,\gamma)$. The $X$-factors cancel in the $2 \times 2$ minor, so $F$ is rank-1 in $(\beta,\gamma)$ for fixed $\alpha$:
\[
...
\medskip\noindent\textbf{Step 4.}\quad By $(\star_{34})$, for fixed $(\gamma,\delta)$, $\lambda$ is rank-1 in $(\alpha,\beta)$. The $H$ and $X$ factors cancel: $G(\alpha_1,\beta_1)\, G(\alpha_2,\beta_2) = G(\alpha_1,\beta_2)\, G(\alpha_2,\beta_1)$. So $G$ is rank-1:
\[
...
\medskip\noindent\textbf{Step 5.}\quad By $(\star_{24})$, for fixed $(\beta,\delta)$, $\lambda$ is rank-1 in $(\alpha,\gamma)$. The bracket $g'(\beta)\, X(\beta,\delta)$ is a scalar, so $g(\alpha)\, H(\alpha,\gamma)$ must be rank-1 in $(\alpha,\gamma)$. The $g$-factors cancel, so $H$ is rank-1:
\[
...
\medskip\noindent\textbf{Step 6.}\quad By $(\star_{13})$, for fixed $(\alpha,\gamma)$, $\lambda$ is rank-1 in $(\beta,\delta)$. The bracket $g(\alpha)\, h(\alpha)\, w(\gamma)$ is a scalar, so $g'(\beta)\, X(\beta,\delta)$ must be rank-1 in $(\beta,\delta)$. The $g'$-factors cancel, so $X$ is rank-1:
\[
...
\begin{remark}
The rank-1 tensors form the Segre variety $\Seg(\Pp^{n-1} \times \Pp^{n-1} \times \Pp^{n-1} \times \Pp^{n-1})$, cut out by $2 \times 2$ minors of all mode flattenings \cite{La12}. Proposition~\ref{prop:rank1} shows that the six pairwise rank-1 conditions $(\star_{12})$--$(\star_{34})$---which are precisely the $2 \times 2$ minors of the six $\binom{4}{2}$ flattenings---suffice to place $\lambda$ on the Segre variety (restricted to the non-identical support).
\end{remark}
...
The polynomial map $\F$ consists of six families of Pl\"ucker equations $\mathcal{P}_{12}, \mathcal{P}_{13}, \mathcal{P}_{14}, \mathcal{P}_{23}, \mathcal{P}_{24}, \mathcal{P}_{34}$ (degree~4 each), totaling $O(n^6 \cdot 3^{12})$ equations. The proof establishes:
\begin{enumerate}
\item \textbf{Camera-independence}: $\F$ is expressed purely in terms of $T$-entries.
\item \textbf{Bounded degree}: all coordinate functions have degree~4, independent of $n$.
\item \textbf{Exact characterization} (for Zariski-generic cameras): $\F(\lambda \cdot Q) = 0 \iff \lambda$ is rank-1.
\end{enumerate}

The assumption $n \geq 5$ is used in two places: (i)~the Pl\"ucker equations require six camera slots (two shared, four varying), and for $n = 5$ some varying cameras must repeat a shared camera---this is valid since the problem defines $\lambda_{\alpha\beta\gamma\delta} \neq 0$ for non-identical (not all-same) tuples, so e.g.\ $(\alpha,\beta,\gamma,\alpha)$ is permitted; (ii)~the factor extraction in Proposition~\ref{prop:rank1} requires five distinct reference indices $a_0, b_0, c_0, d_0, e_0$. The genericity witness in Lemma~\ref{lem:genericity} applies equally to $n = 5$ (with $v_4 = c_p$), as verified by explicit computation. For $n \leq 4$, the problem is open under this proof technique. \qed
...
Ch.~1, \S5, eq.~(5.1), p.~211: Pl\"ucker relations for $\Gr(k,n)$.

\bibitem{HP94}
W.V.D.~Hodge, D.~Pedoe,
\emph{Methods of Algebraic Geometry}, Vol.~I,
Cambridge University Press, 1994 (reprint of 1947 edition).
Ch.~VII, \S4, Theorem~I: quadratic $p$-relations.

\bibitem{HZ04}
R.~Hartley, A.~Zisserman,
\emph{Multiple View Geometry in Computer Vision},
Cambridge University Press, 2nd ed., 2004.
Ch.~17: quadrifocal tensor structure.

\bibitem{Ki17}
J.~Kileel,
``Algebraic Geometry for Computer Vision,''
PhD thesis, UC Berkeley, 2017.
Quadrifocal tensors and multiview variety.

\bibitem{La12}
J.M.~Landsberg,
\emph{Tensors: Geometry and Applications},
AMS Graduate Studies in Mathematics, Vol.~128, 2012.
Ch.~4: Segre variety and tensor rank.
